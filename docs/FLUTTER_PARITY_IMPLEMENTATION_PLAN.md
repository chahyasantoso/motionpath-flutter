...
- Verified: added controller coverage for per-track isolation, whole-graph fallback, untouched core `patches` snapshot under filtered composition, zero-listener gating, imperative `seek()`/`publish()`, single composition per tick, unchanged-patch suppression, deep equality of nested filter/instance payloads, and notifier lifecycle. No local Dart/Flutter run was executed here; CI is the gate.
- Result: partial. Four Phase 3 checklist items closed. The gate stays open.
- Risks: an interest-scoped composition intentionally leaves unwatched entries in `MotionPathPatchController.patches` stale, and does not fire the host `onPatches` hook. Both are documented; a consumer needing every track every frame must keep a whole-graph listener. Track vanishing is currently only observable through graph composition because a motion's track list is immutable, so pruning is defensive rather than exercised by a real removal path.
- Next: z/perspective/3D in the shared transform resolver, an explicit unsupported-key policy, the image cache and disposal contract, bounded filter composition with invalid-sigma coverage, renderer performance tests, then the Spiral demo migration.

### 2026-07-31: Phase 3, shared renderer payload consumption

- Changed: `MotionPathPatchView` now applies patch-driven color and visibility and exposes host-owned image frame, CSS variable, and instance builders, while keeping the transform, opacity, blur, and diagnostic paths intact.
- Verified: widget coverage for the new payload consumers plus the restored renderer regressions; CI ran the Flutter adapter job.
- Result: partial, merged as PR #73. The first CI run failed because `Offstage` wrapped the effect chain and widget finders skip offstage subtrees, and because `ColorFiltered` and `Offstage` were conditional wrappers that remount the supposedly stable child. Both wrappers are now unconditional identity-safe wrappers with `Offstage` applied closest to the child.
- Risks: conditional wrappers are the recurring failure mode in this widget. Any future payload consumer must degrade to an identity wrapper rather than disappearing from the tree.
- Next: the interest-scoped consumer slice, then the remaining 3D, image cache, filter bounds, performance, and demo migration work.
...
| 3 Shared Flutter renderer | Partial | Reusable child wrapper with a stable `AnimatedBuilder.child` chain, shared 2D transform resolver, ARGB and degree-to-radian conversion, patch-driven color and visibility, host-owned image/CSS/instance builders, blur consumer, diagnostic painter, interest-scoped `trackPatch()` consumers with whole-graph fallback, zero-listener tick gating, one composition per update, deep dirty checking, and renderer tests. | No z, perspective, or 3D resolution. No explicit unsupported-key policy. Image resolution has no cache or disposal contract. Filter composition is unbounded and lacks invalid-sigma coverage. No renderer performance test. The Spiral demo still recomputes visual state locally. |
...
- Separated advancement from composition in the core runtime and removed the controller's double composition per tick.
- Added interest-scoped per-track patch consumers, zero-listener tick gating, explicit notifier release, vanished-track pruning, and deep patch dirty checking.
- Recorded that Phase 3 remains open: z/perspective/3D, unsupported-key policy, image cache and disposal, bounded filter composition, performance tests, and Spiral demo migration are still outstanding.
- Landed patch-driven color and visibility plus host-owned image, CSS, and instance builders, and fixed the conditional-wrapper regression that hid the effect chain behind `Offstage`.
