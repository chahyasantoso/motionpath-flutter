...
| 6 Cross-repository parity | Partial | PRs #91 through #105 cover authored payloads, path behavior, triggers, repeat boundaries, anchors, validation diagnostics, lifecycle controls, completion events, and plugin edge contracts. PR #106 adds whole-timeline trajectory fixtures. PRs #107 through #109 add observation graph, lifecycle, and repeat/stagger fixture matrices. PR #110 adds the malformed-project diagnostics matrix. Remaining: `filter` and CSS variable plugin fixtures, fixture tooling, and the eased-overshoot divergence. | Open. See `docs/PHASE_6_ACCEPTANCE.md` |
