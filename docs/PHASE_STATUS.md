...
| 3 Shared Flutter renderer | Active | PR #73 merged: patch-driven color and visibility, host-owned image/CSS/instance builders, and a stable wrapper chain. Interest-scoped per-track consumers, zero-listener tick gating, single composition per update, and deep dirty checking follow in the current slice. Still open: z/perspective/3D resolution, explicit unsupported-key policy, image cache and disposal, bounded filter composition, performance tests, and Spiral demo migration. | Open. Must finish every renderer task and pass CI before Phase 4 |
