...
| 4 Dynamic children and spawn lifecycle | Active | PRs #81 through #85 merged: deterministic ordering, top-most-first helper, redundant Spiral rebuild removed, restart/shared-ticker lifecycle coverage, stable Stack paint order, and reusable front-most hit-test selector. This slice wires that selector into `MotionPathSpawnView` through an opt-in host callback. | Open. Reflow, drain, host hit testing, and final lifecycle cleanup still need full exit evidence |
