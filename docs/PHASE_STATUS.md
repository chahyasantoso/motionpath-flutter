...
| 4 Dynamic children and spawn lifecycle | Active | PR #81 merged deterministic ordering and top-most-first hit-test helpers. This slice removes Spiral's redundant `setState`, leaving `MotionPathSpawnController` as the lifecycle notifier and the single ticker as the frame source. | Open. Reflow, drain, ordering, hit testing, and lifecycle cleanup still need full exit evidence |
