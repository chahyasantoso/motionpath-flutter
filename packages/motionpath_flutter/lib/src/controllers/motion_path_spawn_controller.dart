...
class MotionPathSpawnController extends ChangeNotifier {
  ...
  ///
  /// The caller owns wave policy. This method only resets timing and republishes
  /// the empty state, so the next child is mounted at the path start rather than
  /// being born after an old elapsed timeline and immediately draining again.
  ...
  void _drain() {
    ...
    if (completed.isNotEmpty) _seekChildren();
  }
