...
class MotionPathSpawnController extends ChangeNotifier {
  ...
  ///
  /// The caller owns wave policy. This only resets timing and republishes the
  /// empty state, so the next child starts at the path origin instead of being
  /// born after an old elapsed timeline and immediately draining again.
  ...
  double _spanOf(MotionPathTrackRuntime child) {
    if (child.duration > 0) return child.duration;
