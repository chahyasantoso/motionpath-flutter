        ...
        <String, Object?>{
          ...
          'tracks': <Object?>[
            ...
            <String, Object?>{
              'id': 'spine',
              'observes': <Object?>[
                <String, Object?>{'source': 'pelvis', 'role': 'input', 'target': 'parentWorld'},
              ],
              'keyframes': <String, Object?>{
...
void main() {
  group('renderer boundary units', () {
    test('reads patch rotation as authored degrees', () {
      final MotionPathPatchTransform transform =
          MotionPathPatchTransform.fromPatch(const <String, Object?>{'rotation': 180});
    ...
    test('builds a rotation matrix from degrees, not radians', () {
      final List<double> storage =
          const MotionPathPatchTransform(rotation: 90).toMatrix4Storage().toList();
  ...
  group('walker segments', () {
    ...
    test('paints every drawn bone of the default rig once', () {
      ...
      final List<MotionPathWalkerSegment> segments = resolveWalkerSegments(patches);
  ...
  group('walker scene', () {
    testWidgets('an engine tick drives the rig without a rebuild',
      ...
      await tester.pumpWidget(
        Center(child: MotionPathWalkerScene(source: source, size: const Size(760, 300))),
      );
      engine.tick(0.5);
      await tester.pump();
    ...
    testWidgets('paints an empty source without throwing',
      ...
      await tester.pumpWidget(Center(child: MotionPathWalkerScene(source: source)));
      expect(tester.takeException(), isNull);
    });

    test('repaints only when the rig description changes', () {
      final MotionPathPatchSource source = MotionPathPatchSource();
      final MotionPathWalkerPainter painter = MotionPathWalkerPainter(source: source);
      expect(painter.shouldRepaint(MotionPathWalkerPainter(source: source)), isFalse);
      expect(
        painter.shouldRepaint(
          MotionPathWalkerPainter(source: source, joints: const <MotionPathWalkerJoint>[]),
        ),
        isTrue,
      );
    });
