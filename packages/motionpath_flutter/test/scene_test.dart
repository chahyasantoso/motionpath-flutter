        ...
        <String, Object?>{
          ...
          'tracks': <Object?>[
            ...
            <String, Object?>{
              'id': 'bone',
              'observes': <Object?>[
                <String, Object?>{'source': 'root', 'role': 'input', 'target': 'parentWorld'},
              ],
              'keyframes': <String, Object?>{
...
void main() {
  test('maps scroll offsets onto normalized progress', () {
    const MotionPathScrollBinding binding = MotionPathScrollBinding(start: 100, end: 500);
  ...
  test('eases toward the target only when scrub is authored', () {
    const MotionPathScrollBinding smoothed = MotionPathScrollBinding(scrub: 0.5);
  ...
  testWidgets('a scroll driver reports progress from a controller', (WidgetTester tester) async {
  ...
  testWidgets('publishing a patch repaints the bound view', (WidgetTester tester) async {
    final MotionPathPatchSource source = MotionPathPatchSource();
    await tester.pumpWidget(Center(child: MotionPathPatchView(source: source, trackId: 'pelvis')));
  ...
  testWidgets('an engine tick drives composed FK patches into the painter', (WidgetTester tester) async {
    final MotionPathEngine engine = MotionPathEngine()..loadProject(MotionPathProject.fromJson(_project()));
    ...
    await tester.pumpWidget(Center(child: MotionPathPatchView(source: source, trackId: 'bone')));
    engine.tick(0.5);
    await tester.pump();
    expect((source.patchFor('bone')['x']! as num).toDouble(), closeTo(60, 1e-9));
