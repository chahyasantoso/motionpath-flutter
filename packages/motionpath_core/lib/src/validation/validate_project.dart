...
List<MotionPathDiagnostic> validateProject(Map<String, Object?> json) {
  ...
  for (int index = 0; index < rawMotions.length; index++) {
    ...
    final MotionPathMotion parsed = MotionPathMotion.fromJson(
      motion,
      templates: templates,
    );
    for (int trackIndex = 0; trackIndex < parsed.tracks.length; trackIndex++) {
      ...
      diagnostics.addAll(
        trackKeyframeRules(trackJson, '$motionPath.tracks[$trackIndex]'),
      );
      diagnostics.addAll(
        easingRules(trackJson, '$motionPath.tracks[$trackIndex]'),
      );
    }
