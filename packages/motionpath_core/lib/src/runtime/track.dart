...
class MotionPathTrackRuntime {
  ...
  Map<String, Object?> snapshot() {
    ...
    for (final MapEntry<String, List<MotionPathStop>> entry
        in authored.entries) {
      raw[entry.key] = interpolateStops(entry.value, progress);
    }
...
List<MotionPathStop> stopsFromKeyframe(Object? raw) {
  ...
  final Object? propertyEase = config['ease'];
  ...
  for (final Object? candidate in rawStops) {
    ...
    final Object? ease = stop['ease'] ?? propertyEase;
    result.add(MotionPathStop(
      progress: progress is num ? progress.toDouble() : 0,
      value: stop['v'],
      ease: MotionPathEasing.resolve(ease is String ? ease : null),
    ));
  }
...
Map<String, List<MotionPathStop>> propertiesFromTrack(MotionPathTrack track) {
  ...
  for (final MapEntry<String, Object?> entry in track.keyframes.entries) {
    final List<MotionPathStop> stops = stopsFromKeyframe(entry.value);
    if (stops.isNotEmpty) {
