...
Map<String, Object?>? _composePath(Map<String, Object?> raw) {
  ...
  final double progress = raw['progress'] is num
      ...
      : 0;
  final double pacedProgress = _pacedProgress(authored, progress);
  return _samplePath(nodes, pacedProgress, autoRotate: _authoredAutoRotate(authored));
}
...
/// Applies the path keyframe's stop easing before physical-distance sampling.
///
/// The JS path plugin receives the synthetic path progress produced by the
/// authored stops. Reading the raw playhead skipped that pacing and made a
/// path with `power1.in` travel at the wrong positions even though ordinary
/// properties respected the same ease.
