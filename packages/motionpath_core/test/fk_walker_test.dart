...
void main() {
  group('forward kinematics math', () {
    ...
    test('folds bone data into a flat renderer patch', () {
      ...
      expect(
        stripInternalPatchKeys(patch).keys,
        isNot(contains('parentWorld')),
      );
  ...
  group('walker rig', () {
    ...
    test('parses fourteen tracks and compiles a parent-first order', () {
      ...
      expect(
        order.indexOf('leg-near-foot'),
        greaterThan(order.indexOf('leg-near-shin')),
      );
    ...
    test('keeps every joint at its authored bone length through the gait', () {
      for (final double p in _samples) {
        ...
        expect(
          _span(patches['pelvis']!, patches['chest']!),
          closeTo(kTorso, 1e-6),
        );
        for (final String side in <String>['near', 'far']) {
          expect(
            _span(patches['pelvis']!, patches['leg-$side-shin']!),
            closeTo(kThigh, 1e-6),
          );
          expect(
            _span(patches['leg-$side-shin']!, patches['leg-$side-foot']!),
            closeTo(kShin, 1e-6),
          );
          expect(
            _span(patches['chest']!, patches['arm-$side-fore']!),
            closeTo(kUpperArm, 1e-6),
          );
        }
      }
    });
    ...
    test('bends the knee across the gait', () {
      ...
      expect(
        angles.reduce(math.max) - angles.reduce(math.min),
        greaterThan(10),
      );
    });
  ...
  group('graph publisher', () {
    test('composes parents first but publishes only dirty tracks', () {
      ...
      final Map<String, Map<String, Object?>> published = publisher.flush(
        graph.order,
      );
