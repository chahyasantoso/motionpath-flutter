...
void main() {
  test('supports delay and yoyo progress', () {
    const MotionPathTrigger trigger =
        MotionPathTrigger(delay: 1, yoyo: true, repeat: 1);
  ...
  test('reads trigger settings from authored JSON', () {
    final MotionPathTrigger trigger =
        MotionPathTrigger.fromJson(<String, Object?>{
      'type': 'scroll',
      'scrub': true,
      'autoplay': true,
    });
  ...
  test('flushes dirty tracks in compiled order', () {
    ...
    final MotionPathGraphPublisher publisher =
        MotionPathGraphPublisher(<String, MotionPathTrackRuntime>{
      'root': root,
      'child': child,
    });
    ...
    expect(
      publisher.flush(<String>['root', 'child']).keys,
      <String>['root', 'child'],
    );
