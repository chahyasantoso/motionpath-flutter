...
class _SpiralZumaPageState extends State<SpiralZumaPage>
  ...
  void _syncVisuals(double delta) {
    ...
    for (final MotionPathSpawnInstance instance in _spawns.instances) {
      ...
      visual.entranceAge = (visual.entranceAge + delta)
          .clamp(0, _entranceDuration)
          .toDouble();
      visual.reflowAge = (visual.reflowAge + delta)
          .clamp(0, _reflowDuration)
          .toDouble();
      visual.progress =
          ((_spawns.elapsed - _displayOffset(visual)) / _ballDuration)
              .clamp(0.0, 1.0)
              .toDouble();
  ...
  double _displayOffset(_BallVisual visual) {
    ...
    return visual.offset +
        (visual.targetOffset - visual.offset) *
          ...
          child: GestureDetector(
            onTapUp: (TapUpDetails details) {
              ...
              for (final MotionPathSpawnInstance instance
                ...
                final double d =
                    (_position(visual.progress) - details.localPosition)
                        .distance;
                if (d < distance) {
                  distance = d;
                  nearest = instance.id;
...
class _SpiralPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    canvas.drawRect(
      Offset.zero & size,
      Paint()..color = const Color(0xFF08070D),
    );
    ...
    for (final MotionPathSpawnInstance instance in instances) {
      ...
      final double entrance =
          (visual.entranceAge / _SpiralZumaPageState._entranceDuration).clamp(
            0.0,
            1.0,
          );
      _drawBall(
        canvas,
        _pathPosition(path, visual.progress),
        entrance,
        1 + 0.7 * math.sin(math.pi * entrance),
        visual.progress,
      );
    }
    for (final _BallVisual visual in exiting) {
      final double t = (visual.exitAge / _SpiralZumaPageState._exitDuration)
          .clamp(0.0, 1.0);
  ...
  void _drawBall(
    ...
    canvas.drawCircle(
      point,
      _SpiralZumaPageState._ballRadius * scale,
      Paint()..color = _withOpacity(base, opacity),
    );
    canvas.drawCircle(
      point + const Offset(-6, -6),
      5 * scale,
      Paint()..color = _withOpacity(Colors.white, opacity),
    );
  }
